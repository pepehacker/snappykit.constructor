...
export const isUrl = (message: string = 'Incorrect URL!') => (value: string): Object => ({
  message,
  isValid: value && /^http:\/\/\w+(\.\w+)*(:[0-9]+)?\/?(\/[.\w]*)*$/.test(value),
});

export const matches = (regex: Object<RegExp>, message: string = 'Incorrect value!') => (
  value: string,
): Object => ({
  message,
...
export const max = (max: number, message: string = 'Max value: %d!') => (
  ...
  isValid: !value || typeof value === 'string' ? value.length <= max : parseFloat(value, 10) <= max,
});
...
export const min = (min: number, message: string = 'Min value: %d!') => (
  ...
  isValid: !value || typeof value === 'string' ? value.length >= min : parseFloat(value, 10) >= min,
});
