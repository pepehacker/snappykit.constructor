  ...
  return api(!isAppStore && 'store.getPlayData', {
    ...
    ({ data }): void => {
      const newWebsite = {
        ...
        description: isAppStore ? get(result, 'description') : get(data, 'content'),
        link: isAppStore ? get(result, 'trackViewUrl') : get(result, 'link'),
        logo: get(result, 'logo'),
        provider: get(result, 'provider'),
        screenshots: isAppStore ? get(result, 'screenshotUrls') : get(data, 'images'),
...
export const search = ({ store, ...values }): func => (
  ...
  return api('store.search', { store, ...values })
    .then(({ data }) => {
      ...
      const results = store === STORE_APPLE_ID ? get(data, 'results', []) : data;

      const formatedData = results.map((result: Object) => ({
        ...result,
        description: store === STORE_APPLE_ID ? get(result, 'description') : get(result, 'id'),
        id: store === STORE_APPLE_ID ? get(result, 'bundleId') : get(result, 'id'),
        logo: store === STORE_APPLE_ID ? get(result, 'artworkUrl512') : get(result, 'logo'),
        provider: store,
        title: store === STORE_APPLE_ID ? get(result, 'trackName') : get(result, 'title'),
      }));

      dispatch({ type: SEARCH_SUCCESS, query, result: formatedData });
    })
    .catch((error: Object) => dispatch({ type: SEARCH_FAILURE, error: get(error, 'message') }));
};
