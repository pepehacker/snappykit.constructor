...
const domain =
  window.location.hostname === 'localhost' ? 'sjimml.snappykit.com' : window.location.hostname;
...
api('websites.getApp', { domain }, { noCredentials: true }).then(({ data: app }) => {
  try {
    ...
    const fonts = uniq(
      values(get(data, 'section', {})).map((section: Object): string => get(section, 'font')),
    )
    ...
    ReactDOM.render(
      <Fragment>
        <Helmet>
          <link
            href={`https://fonts.googleapis.com/css?family=${fonts.join('|')}:300,400,500,700`}
            rel="stylesheet"
          />
        ...
        <Router history={history}>
          <Route
            path="/"
            render={props => <Sandbox
              {...props} app={formattedApp}
